 ...
 *
 * <p>Sticks: Right Stick = Drive Stick Left Stick = Drive Stick Right Stick Press = Toggle PIDs
 * Left Stick Press = Toggle Compressor
 *
 * <p>Buttons: Start Button = Reset Encoders and Gyro Back Button = Cancel any Command
 *
 * <p>Bumpers/Triggers:
 *
 * <p>POV: Any Angle = Rotate to the Pressed Angle
 */
public class OI extends SubsystemBase {
  ...
  public boolean cargoEject() {
    ...
    return false; // driverController.getYButton();
  }
  ...
  public void periodic() {
    ...
    if (driverController.getPOV() >= 0) {
      ...
      armLevelSetPoint = 1;
      armManualDriveMode = false;
    } else if (driverController.getLeftBumper() && false) {
      armLevelSetPoint = 2;
      armManualDriveMode = false;
    } else if (driverController.getRightBumper() && false) {
      armLevelSetPoint = 4;
      armManualDriveMode = false;
    }
    if (getArmUp() > 0) {
  ...
  public void configureButtonBindings(
    ...
    new Trigger(() -> getCancelCommand())
        .onTrue(
            new CancelCommand(this, driveSubsystem, cargoSubsystem, hatchSubsystem, liftSubsystem));

    /*
     * Hatch Commands
     */
    new Trigger(() -> getHatchSlideCentre()).onTrue(new HatchCenterCommand(hatchSubsystem));

    new Trigger(() -> getHatchRocketEject()).onTrue(new HatchEjectRocketCommand(hatchSubsystem));
